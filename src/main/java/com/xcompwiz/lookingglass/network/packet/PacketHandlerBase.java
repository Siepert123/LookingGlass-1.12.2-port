...
public abstract class PacketHandlerBase {
     ...
     */
    public static ByteBuf createDataBuffer(Class<? extends PacketHandlerBase> clazz) {
        ByteBuf data = Unpooled.buffer();
        data.writeByte(LookingGlassPacketManager.getId(clazz));
        return data;
    }
    ...
    protected static FMLProxyPacket buildPacket(ByteBuf payload) {
        return new FMLProxyPacket(new PacketBuffer(payload), LookingGlassPacketManager.CHANNEL);
    }

    /**
     * Used by the progeny of this class in order to produce a packet object from the data buffer. Automatically uses our packet channel so that the manager on
     * the other side will receive the packet. Uses the packet buffer directly instead of a wrapped ByteBuf.
     */
